...
<!-- Body -->
	<div class="library-banner">
		  ...
		  <h1>Here are all the books that we have in this library</h1>
...
<div class="container">
	...
	<% ArrayList<Book> allBooks = (ArrayList<Book>)session.getAttribute("allBooks");
				for(Book b : allBooks) { %>
	<div class="flip-card book">
	  <div class="flip-card-inner">
	    ...
	    <div class="flip-card-back">
	    	<%out.print("<form action='/ncsLibrary/AddToFav'> <input class='hidden d-none' type='text' name='bookId' value='"+b.getBookId()+"'> <button type='submit' class='fav-btn'> <i class='fa-solid fa-heart'></i> </button></form>"); %>
	      <h5>Book Summary</h5>
					<%out.print("<p class='card-text text-justify p-3'>" + b.getDescription() +"</p>");%>
					<%if(b.getBorrowed().equals("true")){out.print("<p style='background-color: indianred;'> On Loan </p>");}
 					else {out.print("<form action='/ncsLibrary/BorrowBook'> <input class='hidden d-none' type='text' name='bookId' value='"+b.getBookId()+"'> <input type='submit' class='borrow-btn' value='Borrow'></form>");}%>
	    </div>
	  </div>
	</div>
			<%} %>
